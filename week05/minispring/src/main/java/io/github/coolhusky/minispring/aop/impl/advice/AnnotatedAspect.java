...
import io.github.coolhusky.minispring.annotations.After;
import io.github.coolhusky.minispring.annotations.Around;
import io.github.coolhusky.minispring.annotations.Aspect;
import io.github.coolhusky.minispring.annotations.Before;
...
import io.github.coolhusky.minispring.utils.AssertUtils;
 ...
 * @author jcwang
 */
public class AnnotatedAspect implements io.github.coolhusky.minispring.aop.Aspect {
    ...
    private final Map<String, Pointcut> POINT_CUT_CACHE = new ConcurrentHashMap<>(16);

    public AnnotatedAspect(String name, Class<?> aspectClass) {
        this.aspectClass = aspectClass;
        this.name = name;
        AssertUtils.isTrue(() -> aspectClass.isAnnotationPresent(Aspect.class),
                new MiniSpringRuntimeException("must be annotated as @minispring.annotations.Aspect"));
        Aspect annotation = aspectClass.getAnnotation(Aspect.class);
...
//        retrieveAdvisors();
    ...
    private void retrieveAdvisors() {
        ...
        // find pointcut
        Stream.of(methods)
                .filter(m -> m.isAnnotationPresent(io.github.coolhusky.minispring.annotations.Pointcut.class))
                .map(m -> {
                    io.github.coolhusky.minispring.annotations.Pointcut pc = m.getAnnotation(io.github.coolhusky.minispring.annotations.Pointcut.class);
